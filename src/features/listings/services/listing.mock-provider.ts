...
function matches(listing: Listing, params: ListingSearchParams) {
  ...
  if (
    ...
    !accommodationTypes.includes(listing.accommodationType)
  )
    return false;
  if (
    typeof maxDistanceKm === "number" &&
    listing.location.distanceFromCampusKm > maxDistanceKm
  )
    return false;
...
function sortListings(listings: Listing[], sort: ListingSearchParams["sort"]) {
  ...
  switch (sort) {
    ...
    case "distance":
      return copy.sort(
        (a, b) =>
          a.location.distanceFromCampusKm - b.location.distanceFromCampusKm,
      );
    case "newest":
      return copy.sort(
